    ...
    def project(self, X):
        if self.w is not None: ## linear case
            ...
            for i in range(len(X)):
                temp_mat = np.zeros(4)
                temp_mat[0] = X.iloc[i, 0]
                temp_mat[1] = X.iloc[i, 1]
                temp_mat[2] = X.iloc[i, 2]
                temp_mat[3] = X.iloc[i, 3]
                temp = np.dot(self.w.T, temp_mat) + self.b
                predict_y[i] = temp
            return predict_y
            ...
            for i in range(len(X)):
                temp = 0
                k = self.kernel(X) + 0.000001 * np.diag(np.random.rand(274,274))
                for j in range(len(self.a)):
                    temp += np.dot(self.a[j], self.sv_y[j]) * k[j][i]
                    y_predict[i] = temp
